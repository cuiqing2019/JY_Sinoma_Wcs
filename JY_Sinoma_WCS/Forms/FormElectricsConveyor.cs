...
{
    public partial class FormElectricsConveyor : Form
    {
        private ElectricsStationInConveyor conveyor;

        SystemStatus systemstatus;
        public ConnectPool dbConn;
        public int index;
        public FormElectricsConveyor(ElectricsStationInConveyor electric, int nIndex, SystemStatus systemStatus)
        {
            InitializeComponent();
            ...
            if (electric.isBindToPLC)
            {
                ...
                if (electric.loadStruct[nIndex].loadType>0)
                    lbLoadStatus.Text += "有货；";
                else
        ...
        private void btWriteId_Click(object sender, EventArgs e)
        {
            ...
            conveyor.WriteID(index, int.Parse(this.tbWriteId.Text.ToString()), cmbTaskType.SelectedIndex, int.Parse(this.txtFrom.Text.ToString()), int.Parse(this.txtTo.Text.ToString()), cmbLoadType.SelectedIndex);
        }
    }
}
