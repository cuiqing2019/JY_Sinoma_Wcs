...
{
    public class ConveyorWeightLoad : ConveyorLoad
    {

        public string[] weightDB;//对应辊道的重量DB块地址
        public int[] weightHandle;//辊道控制的OPCServer的Handle 

        public double[] goodsWeight = new double[2] { 0, 0 };//货物重量
        ...
        /// <param name="bt"></param>
        public ConveyorWeightLoad(frmMain mainFrm, DataTable bt)
            : base(mainFrm, bt)
        {
            ...
            foreach (DataRow row in bt.Rows)
            {
                ...
                conveyorName[i] = ((row["level_no"].ToString() == "1") ? "RKSS" : "CKSS") + row["device_name"].ToString();
        ...
        public override void RefreshStatus()
        {
            ...
            {
                MessageBox.Show("ConveyorWeightLoad类，RefreshStatus方法：" + ex.Message);
        ...
        public override void lb_DoubleClick(object sender, EventArgs e)
        {
            ...
            FormConveyorLoad frmConveyor = new FormConveyorLoad((ConveyorLoad)this, Index, mainFrm.systemStatus);
        ...
        public override void DisplayStatus()
        {
            try
            {
                for (int i = 0; i < this.nCount; i++)
                {
                    ...
                    lb[i].Text = "重量:" + goodsWeight[i].ToString();
                }
            }
            catch (Exception ex)
            {
