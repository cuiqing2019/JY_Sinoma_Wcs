...
{
    public class ElectricsConveyorAll : Conveyor
    {
        #region 自定义变量
        ...
        /// <param name="bt"></param>
        public ElectricsConveyorAll(frmMain mainFrm, DataTable bt)
            : base(mainFrm, bt)
        {
        ...
        public override void lb_DoubleClick(object sender, EventArgs e)
        {
            ...
            FormElectricsConveyorAll frmConveyor = new FormElectricsConveyorAll(this, Index, mainFrm.systemStatus);
                    ...
                    try
                    {
                        if (mainFrm.bPicking)
                        {
                            ...
                            if (this.loadStruct[nScannerId].loadType != 0 && mainFrm.bPicking)
                            {
                                ...
                                DataRow dr2 = DataBaseInterface.SelectTaskD(conn, (loadStruct[nScannerId].taskID-1000).ToString(), "CON");
                                if (dr2 != null && int.Parse(dr2["task_type"].ToString()) == loadStruct[nScannerId].taskType)
                                {
                                    int i = 0;
                                    switch (loadStruct[nScannerId].taskType)
                                    {
                                        case 3://空托盘入库
                                            i = DataBaseInterface.TaskStatusUpdate(loadStruct[nScannerId].taskID-1000, nDeviceID[nScannerId], conveyorName[nScannerId], int.Parse(dr2["step"].ToString()), 2, out rs);
                                            if (i != 1)
                                            {
                                                MessageBox.Show("堆垛机取货站台空托入库辊道任务报完成失败！" + rs);
                                            }
        ...
        public override void WriteCmd(int nChannel, int nTaskID, int nTaskType, int FromLev, int ToLev, int loadType)
        {
            if (nChannel < 0 || nChannel > 8)
                return;
            if (GetTask(nChannel) == nTaskID)//返回的任务号和下载的任务号相同 则停止写任务
                return;
            int[] handle = new int[1];
            handle[0] = writeHandle[nChannel];
